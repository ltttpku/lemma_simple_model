...
# from datautils import lemma_qa
from datautils.lemma_qa import *
...
def extract_clips_with_consecutive_frames(path, num_clips, num_frames_per_clip, lemma_qa=False):
    """
    Args:
    ...
    if lemma_qa:
        ...
        try:
            # # video_data: numpy.ndarry, (432, 240, 320, 3), (N_frames, H, W, C), dtype=uint8(0~255)
...
def generate_h5(model, video_ids, num_clips, outfile, lemma_qa=False):
    """
    Args:
    ...
    with h5py.File(outfile, 'w') as fd:
        ...
        for i, video_path in enumerate(video_ids):
            ...
            _t['misc'].tic()
            clips, valid = extract_clips_with_consecutive_frames(video_path, num_clips=num_clips, num_frames_per_clip=16, lemma_qa=lemma_qa)
            if args.feature_type == 'appearance':
                ...
                clip_feat = np.asarray(clip_feat)  # (8, 16, 2048), num_clips x num_frames_per_clip x 2048
...
if __name__ == '__main__':
    ...
    # dataset info
    parser.add_argument('--dataset', default='tgif-qa', choices=['tgif-qa', 'msvd-qa', 'msrvtt-qa', 'lemma-qa'], type=str)
    ...
    if args.dataset == 'tgif-qa':
        ...
        args.annotation_file = '/home/leiting/scratch/data/msrvtt/annotations/{}_qa.json'
        args.video_dir = '/home/leiting/scratch/data/msrvtt/videos/'
